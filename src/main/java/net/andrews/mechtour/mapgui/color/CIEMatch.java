...
public class CIEMatch {
    ...
    private static double precision = 100000000;;
    ...
    private static double ciedesqrd(double L1, double a1, double b1, double L2, double a2, double b2) {
        ...
        double T = 1 - 0.17 * precision * cosd(hBarPrime - 30) + 0.24 * precision * cosd(2 * hBarPrime)
                + 0.32 * precision * cosd(3 * hBarPrime + 6)
                - 0.2 * precision * cosd(4 * hBarPrime - 63) / precision / precision;

        double slCoeff = (lBar - 50) * (lBar - 50);
        double sl = 1 + 0.015 * precision * slCoeff / Math.sqrt(20 + slCoeff) / precision;
        double sc = 1 + 0.045 * precision * cBarPrime / precision;
        double sh = 1 + 0.015 * precision * cBarPrime * T / precision;
